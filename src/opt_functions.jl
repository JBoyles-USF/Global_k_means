...
function global_OPT_base(X, k, lower=nothing, upper=nothing, mute=false)
    ...
    set_optimizer_attribute(m, "TimeLimit", time_lapse * 16)
    set_optimizer_attribute(m, "MIQCPMethod", 0)   # or remove to let Gurobi auto-choose
...
function global_OPT3_LD(X, k, lambda, ctr_init, w_sos=nothing, lower=nothing, upper=nothing, mute=false, solver="Gurobi")
    d, n = size(X)
    lower, upper = init_bound(X, d, k, lower, upper)
    dmat_max = max_dist(X, d, k, n, lower, upper)
