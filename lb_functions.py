...
# You would import your Python equivalent of `opt_functions` here
import opt_functions  

# tolerance and minimum gap
tol = 1e-6
mingap = 1e-3
...
def getGlobalLowerBound(nodeList):
    LB = 1e15
    nodeid = 0  # Python uses 0-based indexing
    for idx, n in enumerate(nodeList):
        # print("remaining", idx, "  ", n.LB)
        if n.LB < LB:
            LB = n.LB
            nodeid = idx
    return LB, nodeid
...
def getLowerBound_analytic(X, k, lower=None, upper=None):
    ...
    if lower is None or upper is None:
        lower, upper = opt_functions.init_bound(X, d, k, lower, upper)  # keep your opt_functions wrapper
    
    LB = 0
    centers_gp = np.random.rand(d, k, n)
    
    for s in range(n):
        # Repeat column to match k clusters
        x_mat = np.tile(X[:, s].reshape(d, 1), (1, k))
        # Apply element-wise median function along first axis
        mu = np.median(np.stack([lower, x_mat, upper]), axis=0)
        centers_gp[:, :, s] = mu
        
        # Compute min distance from X[:, s] to each mu[:, i]
        min_dist = np.inf
        for i in range(k):
            dist = np.sum((X[:, s] - mu[:, i]) ** 2)
            if dist <= min_dist:
                min_dist = dist
...
def strGrp_nofill(assign, ngroups):
    ...
    for i in l:
        p = np.random.permutation(len(c[i]))  # Shuffle the index to introduce randomness
        for j in p:
            if ng == ngroups:
                ng = 0
            ng += 1
            groups[ng - 1].append(c[i][j])
...
def kmeans_group(X, assign, ngroups):
    np.random.seed(123)
    _, clst_idx = unique_inverse(assign)  # clst_label and clst_idx
    
    groups = [[] for _ in range(ngroups)]
    
    for i in range(len(clst_idx)):
        if len(clst_idx[i]) == 1:  # If the cluster only has one sample, assign it directly to group 1
            groups[0].append(clst_idx[i][0])
        else:
            k_sub = max(2, len(clst_idx[i]) // ngroups)  # Number of sub-clusters per cluster
            n_trial = 5
            mini_cost = float('inf')
            X_sub = X[:, clst_idx[i]]  # Get sub-cluster data
            
            for tr in range(n_trial):
                kmeans = KMeans(n_clusters=k_sub, random_state=123)
                kmeans.fit(X_sub.T)  # Transpose for correct shape
            ...
            clst_groups = strGrp_nofill(assign, ngroups)  # Get grouping label from one cluster
            for j in range(len(clst_groups)):
                groups[j].extend([clst_idx[i][idx] for idx in clst_groups[j]])
